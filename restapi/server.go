...
// NewServer creates a new api flamedb server but does not configure it
func NewServer(api *operations.FlamedbAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/flamedb.sock"`
...
func (s *Server) SetAPI(api *operations.FlamedbAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(2)
		s.Logf("Serving flamedb at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving flamedb at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(2)
		s.Logf("Serving flamedb at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving flamedb at http://%s", l.Addr())
		}(s.httpServerL)
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(2)
		s.Logf("Serving flamedb at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving flamedb at https://%s", l.Addr())
